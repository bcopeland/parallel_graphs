...
typedef adjacency_list<vecS, distributedS<mpi_process_group, vecS>,
                       directedS, property<vertex_distance_t, float>,
...
int main(int argc, char *argv[])
{
    ...
    // make up some distances
    /*
    BGL_FORALL_EDGES(e, g, graph_t)
    {
        float weight = drand48();
        std::cout << get(get(vertex_index, g), source(e, g)) << ","
                  << get(get(vertex_index, g), target(e, g)) << ": "
                  << weight << std::endl;
        put(get(edge_weight, g), e, weight);
    }
    */
    ...
    dijkstra_shortest_paths(g, vstart,
    ...
    for (i=0 ; i < reader.num_vertices(); i++)
    {
        ...
        if (owner(v) == my_id)
        {
            float distance = get(get(vertex_distance, g), v);
